...
const quickActions = [
  { to: '/evaluations/new', label: 'Avaliação', icon: ClipboardPlus, tone: 'bg-heal-softBlue text-heal-blue dark:bg-heal-blue/10' },
...
export function DashboardPage() {
  ...
  return (
    <div className="flex min-h-full min-w-0 flex-1 flex-col bg-white dark:bg-[#0c0c0e]">
      <PageHeader title="Página Inicial" description="Painel de controle clínico" />

      <div className="flex min-w-0 flex-1 flex-col xl:flex-row">
        <div className="order-2 min-w-0 flex-1 xl:order-1 xl:border-r xl:border-heal-line xl:dark:border-zinc-800/60">
          <div className="space-y-6 p-4 sm:p-6 lg:p-7">
            <section className="overflow-hidden rounded-2xl border border-heal-line/75 bg-white p-6 shadow-sm dark:border-zinc-800/85 dark:bg-[#0c0c0e] lg:p-8">
              <div className="max-w-2xl">
                ...
                <p className="mt-3 max-w-xl text-sm leading-6 text-heal-muted dark:text-zinc-400">
                ...
                <div className="mt-5 grid grid-cols-2 gap-3 sm:flex sm:flex-wrap">
                  {quickActions.map(action => (
                    <Link
                      key={action.to}
                      to={action.to}
                      className="inline-flex h-10 min-w-0 items-center justify-center gap-2 rounded-full border border-heal-line/80 bg-white px-3 text-xs font-bold text-heal-ink transition hover:border-heal-blue/50 hover:bg-heal-softBlue/45 dark:border-zinc-800/80 dark:bg-zinc-950/40 dark:text-white sm:justify-start sm:px-4"
                    >
                      <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full ${action.tone}`}>
            ...
            <section className="grid gap-4 sm:grid-cols-2">
              {shortcutCards.map(item => (
                <Link key={item.to} to={item.to} className="group rounded-2xl border border-heal-line/75 bg-white p-5 transition hover:border-heal-blue/40 hover:bg-heal-softBlue/20 dark:border-zinc-800/85 dark:bg-[#0c0c0e] dark:hover:bg-heal-blue/10">
                  <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-heal-softBlue/60 text-heal-blue dark:bg-heal-blue/10">
        ...
        <aside className="order-1 w-full shrink-0 space-y-5 p-4 pb-0 sm:p-6 sm:pb-0 xl:order-2 xl:w-[360px] xl:p-5 xl:pb-5">
          {searchWidget}
          {metricsGrid}
          {appointmentsWidget}
        </aside>
...
function MetricCard({ label, value, icon, tone }: { label: string; value: number; icon: ReactNode; tone: 'blue' | 'green' | 'amber' | 'slate' }) {
  const bgStyles = {
    blue: 'bg-heal-softBlue text-heal-blue dark:bg-heal-blue/10 dark:text-heal-blue',
